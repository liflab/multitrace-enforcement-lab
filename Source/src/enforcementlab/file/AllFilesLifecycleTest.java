...
public class AllFilesLifecycleTest
{
	@Test
	public void test1()
	{
		Quadrilean.Value v; 
		AllFilesLifecycle fl = new AllFilesLifecycle();
		...
		p.push(Event.get("Open 2"));
		v = (Quadrilean.Value) queue.remove();
		assertEquals(Value.P_TRUE, v);
		p.push(Event.get("Close 1"));
		v = (Quadrilean.Value) queue.remove();
		assertEquals(Value.P_TRUE, v);
		p.push(Event.get("Read 2"));
		v = (Quadrilean.Value) queue.remove();
		assertEquals(Value.P_TRUE, v);
		GroupProcessor fl2 = (GroupProcessor) fl.duplicate(true);
